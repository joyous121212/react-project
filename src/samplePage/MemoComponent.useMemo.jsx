...
export const MemoComponent = () => {
  ...
  const filteredDataCount = () => {
    ...
    return data.filter((item) => item.name.toLowerCase().includes(search.toLowerCase())).length;
  };

  const filteredDataCountMemo = useMemo(() => {
    console.log("useMemo를 사용해서 검색 결과를 계산 중...");
    return data.filter((item) => item.name.toLowerCase().includes(search.toLowerCase())).length;
  }, [search]); // search에 대한 값이 변할때만 실행

  return (
    <div>
      <h2>useMemo</h2>
      <input type="number" placeholder="검색어 입력..." value={search} onChange={(e) => setSearch(e.target.value)} />
